    ...
    for (const row of ingredients.rows) {
      ...
      if (item.rows.length) {
        ...
        await tx.query(
          `
          UPDATE items
          SET name = $1, vendor_id = $2, area_type = $3, purchase_unit = $4, purchase_cost = $5, sku = $6, source_system = 'pricebook', source_key = $7
          WHERE id = $8
          `,
          [
            ingredientName,
        ...
        const inserted = await tx.query(
          `
          INSERT INTO items (name, vendor_id, case_size, area_type, purchase_unit, purchase_cost, sku, source_system, source_key)
          VALUES ($1, $2, 1, $3, $4, $5, $6, 'pricebook', $7)
          RETURNING id
          `,
          [ingredientName, vendorId, areaType, purchaseUnit, purchaseCost, normalizeText(row.sku), sourceKey]
        );
