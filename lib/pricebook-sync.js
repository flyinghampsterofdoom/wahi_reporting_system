...
async function upsertTrackedSize(tx, itemId, label, volumeMl, stats) {
  const existing = await tx.query(
  ...
  if (existing.rows.length) {
    ...
    await tx.query("UPDATE item_sizes SET size_label = $1 WHERE id = $2", [label, trackedSizeId]);
    stats.sizesUpdated += 1;
  } else {
    const inserted = await tx.query(
      "INSERT INTO item_sizes (item_id, size_label, volume_ml, is_tracked) VALUES ($1, $2, $3, 0) RETURNING id",
      [itemId, label, volumeMl]
    );
...
async function syncPricebookToCatalog(db) {
  ...
  await db.transaction(async (tx) => {
    const ingredients = await tx.query(`
      SELECT id, ingredient_name, vendor, sku, size_value, purchase_unit, location
    ...
    for (const row of ingredients.rows) {
      ...
      await upsertTrackedSize(tx, itemId, sizeLabel, volumeMl, stats);
