...
test('PostgreSQL migration is explicit, transactional, idempotent and isolated',async()=>{
  ...
  assert.deepEqual(await migrate(pool),['001_domain.sql','002_validation_guards.sql','003_inventory_foundation.sql','004_recipe_categories.sql','005_import_evidence.sql','006_integrations.sql','007_hosted_sessions.sql','008_credential_revocation.sql']);assert.deepEqual(await migrate(pool),[]);await checkSchema(pool);
...
test('hosted PostgreSQL sessions survive store recreation and logout; login limits persist',async()=>{
 const {SessionStore}=require('../deploy/runtime'),crypto=require('node:crypto');const users=[{id:'hosted-owner',salt:'test-salt',hash:'test-hash'}],a=new SessionStore(pool,users);await a.activate();const hash=crypto.randomBytes(32).toString('hex'),session={user:{id:'hosted-owner'},csrf:crypto.randomBytes(24).toString('hex'),expires:Date.now()+60000};await a.set(hash,session);const b=new SessionStore(pool,users);assert.equal((await b.get(hash)).userId,'hosted-owner');await b.delete(hash);assert.equal(await a.get(hash),null);const key=crypto.randomUUID();for(let i=0;i<20;i++)assert.equal(await a.allowLogin(key),true);assert.equal(await b.allowLogin(key),false);
});
...
test('password activation revokes every same-user HTTP session, rejects old password and preserves other users',async()=>{
 const {SessionStore}=require('../deploy/sessions'),{createUser}=require('../review/auth'),{createReviewServer}=require('../review/server'),{DomainService}=require('../service'),{MemoryRepository}=require('../repository/memory'),http=require('node:http'),crypto=require('node:crypto');
 const oldPassword=crypto.randomBytes(24).toString('hex'),newPassword=crypto.randomBytes(24).toString('hex'),otherPassword=crypto.randomBytes(24).toString('hex');
 const a=await createUser('revoke-a','STAFF',oldPassword),b=await createUser('revoke-b','STAFF',otherPassword),replacement={...await createUser('revoke-a','STAFF',newPassword),id:a.id};
 const first=new SessionStore(pool,[a,b]);await first.activate();
 const servers=[];async function start(users,store){const server=createReviewServer({service:new DomainService(new MemoryRepository()),users,sessionStore:store,runtime:{hosted:true,origin:'https://wahi-reporting-system.onrender.com',environment:'production'}});await new Promise(r=>server.listen(0,'127.0.0.1',r));servers.push(server);return (url,{body,cookie,csrf}={})=>new Promise((resolve,reject)=>{const q=http.request({hostname:'127.0.0.1',port:server.address().port,path:url,method:body?'POST':'GET',headers:{Host:'wahi-reporting-system.onrender.com',Origin:'https://wahi-reporting-system.onrender.com','Content-Type':'application/json',...(cookie?{Cookie:cookie}:{}),...(csrf?{'X-CSRF-Token':csrf}:{})}},r=>{let text='';r.on('data',d=>text+=d);r.on('end',()=>resolve({status:r.statusCode,cookie:r.headers['set-cookie']?.[0].split(';')[0],data:JSON.parse(text)}));});q.on('error',reject);q.end(body?JSON.stringify(body):undefined);});}
 try{
  const old=await start([a,b],first),login=(request,username,password)=>request('/api/login',{body:{username,password}});
  const one=await login(old,a.username,oldPassword),two=await login(old,a.username,oldPassword),other=await login(old,b.username,otherPassword);assert.equal(one.status,200);assert.equal(two.status,200);assert.equal(other.status,200);
  const retained=(await pool.query('SELECT * FROM wahi_v2.web_sessions WHERE user_id=$1',[b.id])).rows;
  const next=new SessionStore(pool,[replacement,b]);await next.activate();const current=await start([replacement,b],next);
  for(const session of [one,two]){assert.equal((await current('/api/me',{cookie:session.cookie})).status,401);assert.equal((await old('/api/me',{cookie:session.cookie})).status,401);}
  assert.equal((await login(current,a.username,oldPassword)).status,401);assert.equal((await login(old,a.username,oldPassword)).status,401,'stale process cannot issue sessions after reset');
  assert.equal((await current('/api/me',{cookie:other.cookie})).status,200);assert.deepEqual((await pool.query('SELECT * FROM wahi_v2.web_sessions WHERE user_id=$1',[b.id])).rows,retained);
  const fresh=await login(current,a.username,newPassword);assert.equal(fresh.status,200);const me=await current('/api/me',{cookie:fresh.cookie});assert.equal(me.status,200);
  assert.equal((await current('/api/logout',{cookie:fresh.cookie,csrf:me.data.csrf,body:{}})).status,200);assert.equal((await current('/api/me',{cookie:fresh.cookie})).status,401);
  const events=(await pool.query("SELECT * FROM wahi_v2.auth_security_events WHERE user_id=$1 AND event='password_changed'",[a.id])).rows;assert.equal(events.length,1);assert.equal(events[0].revoked_sessions,2);for(const material of [oldPassword,newPassword,a.hash,a.salt,replacement.hash,replacement.salt,one.cookie,two.cookie])assert.ok(!JSON.stringify(events).includes(material));
  await assert.rejects(pool.query('DELETE FROM wahi_v2.auth_security_events WHERE user_id=$1',[a.id]));
 }finally{for(const s of servers){s.closeAllConnections();await new Promise(r=>s.close(r));}}
});

test('credential activation and audit roll back if session revocation fails',async()=>{
 const {SessionStore}=require('../deploy/sessions'),crypto=require('node:crypto');const user={id:'revoke-failure',salt:'salt',hash:'old'},old=new SessionStore(pool,[user]);await old.activate();const token=crypto.randomUUID();await old.set(token,{user,csrf:'test',expires:Date.now()+60000});
 const before=(await pool.query('SELECT * FROM wahi_v2.auth_credentials')).rows;
 await pool.query("CREATE FUNCTION wahi_v2.test_reject_revoke() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN RAISE EXCEPTION 'test revocation failure'; END; $$; CREATE TRIGGER test_reject_revoke BEFORE DELETE ON wahi_v2.web_sessions FOR EACH ROW EXECUTE FUNCTION wahi_v2.test_reject_revoke()");
 try{await assert.rejects(new SessionStore(pool,[{...user,hash:'new'}]).activate(),/test revocation failure/);}finally{await pool.query('DROP TRIGGER test_reject_revoke ON wahi_v2.web_sessions; DROP FUNCTION wahi_v2.test_reject_revoke()');}
 assert.deepEqual((await pool.query('SELECT * FROM wahi_v2.auth_credentials')).rows,before);assert.ok(await old.get(token));assert.equal((await pool.query("SELECT count(*)::int n FROM wahi_v2.auth_security_events WHERE user_id=$1 AND event='password_changed'",[user.id])).rows[0].n,0);
});

test('audit failure rolls back credential activation and session deletion',async()=>{
 const {SessionStore}=require('../deploy/sessions'),crypto=require('node:crypto');const user={id:'audit-failure',salt:'salt',hash:'old'},old=new SessionStore(pool,[user]);await old.activate();const token=crypto.randomUUID();await old.set(token,{user,csrf:'test',expires:Date.now()+60000});
 await pool.query("CREATE FUNCTION wahi_v2.test_reject_security_event() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN RAISE EXCEPTION 'test security audit failure'; END; $$; CREATE TRIGGER test_reject_security_event BEFORE INSERT ON wahi_v2.auth_security_events FOR EACH ROW EXECUTE FUNCTION wahi_v2.test_reject_security_event()");
 try{await assert.rejects(new SessionStore(pool,[{...user,hash:'new'}]).activate(),/test security audit failure/);}finally{await pool.query('DROP TRIGGER test_reject_security_event ON wahi_v2.auth_security_events; DROP FUNCTION wahi_v2.test_reject_security_event()');}
 assert.ok(await old.get(token));
});

test('concurrent login insertion cannot survive credential activation with an old generation',async()=>{
 const {SessionStore}=require('../deploy/sessions'),crypto=require('node:crypto');const user={id:'concurrent-reset',salt:'salt',hash:'old'},old=new SessionStore(pool,[user]);await old.activate();const next=new SessionStore(pool,[{...user,hash:'new'}]),tokens=Array.from({length:6},()=>crypto.randomUUID());
 const outcomes=await Promise.allSettled([...tokens.map(token=>old.set(token,{user,csrf:'test',expires:Date.now()+60000})),next.activate()]);assert.equal(outcomes.at(-1).status,'fulfilled');
 for(const token of tokens){assert.equal(await old.get(token),null);assert.equal(await next.get(token),null);}await assert.rejects(old.set(crypto.randomUUID(),{user,csrf:'test',expires:Date.now()+60000}),e=>e.code==='unauthenticated');
 assert.equal((await pool.query('SELECT count(*)::int n FROM wahi_v2.web_sessions WHERE user_id=$1',[user.id])).rows[0].n,0);
});

test('unchanged activation preserves existing sessions and adopts pre-fix sessions once; removal revokes access',async()=>{
 const {SessionStore}=require('../deploy/sessions'),crypto=require('node:crypto');const user={id:'adopt-session',salt:'salt',hash:'same'},token=crypto.randomUUID();await pool.query('INSERT INTO wahi_v2.web_sessions(token_hash,user_id,csrf,expires_at) VALUES($1,$2,$3,$4)',[token,user.id,'test',new Date(Date.now()+60000)]);
 const store=new SessionStore(pool,[user]);await store.activate();assert.ok(await store.get(token));const before=(await pool.query('SELECT * FROM wahi_v2.web_sessions WHERE user_id=$1',[user.id])).rows;await store.activate();assert.deepEqual((await pool.query('SELECT * FROM wahi_v2.web_sessions WHERE user_id=$1',[user.id])).rows,before);
 await new SessionStore(pool,[]).activate();assert.equal(await store.get(token),null);assert.equal((await pool.query('SELECT count(*)::int n FROM wahi_v2.auth_credentials WHERE user_id=$1',[user.id])).rows[0].n,0);
});
