...
test('PostgreSQL migration is explicit, transactional, idempotent and isolated',async()=>{
  ...
  assert.deepEqual(await migrate(pool),['001_domain.sql','002_validation_guards.sql','003_inventory_foundation.sql','004_recipe_categories.sql','005_import_evidence.sql','006_integrations.sql','007_hosted_sessions.sql','008_credential_revocation.sql','009_named_users.sql','010_labor.sql']);assert.deepEqual(await migrate(pool),[]);await checkSchema(pool);
