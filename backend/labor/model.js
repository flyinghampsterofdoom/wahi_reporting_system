...
// Latest recorded revision wins at the same effective date; future policy remains intact.
function classificationTimeline(records){
 const sorted=[...records].sort((a,b)=>a.job_guid.localeCompare(b.job_guid)||a.effective_date.localeCompare(b.effective_date)||(a.revision!=null&&b.revision!=null?(BigInt(a.revision)>BigInt(b.revision)?1:BigInt(a.revision)<BigInt(b.revision)?-1:0):String(a.recorded_at||'').localeCompare(String(b.recorded_at||''))));
 const dates=new Map();for(const row of sorted)dates.set(row.job_guid+':'+row.effective_date,row);
 const result=[];for(const row of dates.values()){const prev=result.at(-1);if(prev?.job_guid!==row.job_guid||prev.area!==row.area)result.push(row);}return result;
}
function aggregate(entries,jobs,mappings,target,now){const totals={FOH:Exact.of('0'),BOH:Exact.of('0'),Excluded:Exact.of('0'),Unassigned:Exact.of('0')},detail=new Map();mappings=classificationTimeline(mappings);let provisional=Exact.of('0'),unresolved=0;for(const e of entries){if(e.deleted)continue;const m=mappings.filter(m=>m.job_guid===e.job_guid&&m.effective_date<=e.business_date).sort((a,b)=>b.effective_date.localeCompare(a.effective_date)||String(b.recorded_at).localeCompare(String(a.recorded_at)))[0];const area=m?.area||'Unassigned',key=(e.job_guid||'unknown')+':'+area;let hours;try{hours=worked(e,now);}catch{unresolved++;continue;}totals[area]=totals[area].add(hours.hours);if(hours.provisional)provisional=provisional.add(hours.hours);const row=detail.get(key)||{jobGuid:e.job_guid,name:jobs.find(j=>j.guid===e.job_guid)?.name||'Unknown Toast job',area,hours:Exact.of('0'),entries:0};row.hours=row.hours.add(hours.hours);row.entries++;detail.set(key,row);}
...
module.exports={classificationTimeline,periodFor,date,addDays,businessDate,weekFor,weekEnd,amount,worked,metric,aggregate,fail};
