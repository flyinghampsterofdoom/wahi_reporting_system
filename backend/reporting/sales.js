...
function modifiers(rows,depth=0){if(depth>12||!Array.isArray(rows))throw fail('sales_payload_unsupported');return rows.map(s=>({id:s.guid,itemId:guid(s.item?.guid)?s.item.guid:null,groupId:guid(s.optionGroup?.guid)?s.optionGroup.guid:null,quantity:decimal(s.quantity),price:decimal(s.price),voided:!!s.voided,type:s.selectionType||'NONE',preModifierId:guid(s.preModifier?.guid)?s.preModifier.guid:null,refundAmount:s.refundDetails?decimal(s.refundDetails.refundAmount):null,modifiers:modifiers(s.modifiers||[],depth+1)}));}
