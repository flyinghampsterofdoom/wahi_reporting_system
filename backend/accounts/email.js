...
class AccountEmail {
 ...
 async issue(c,u,purpose,actor){
  ...
  const deliveryId=await this.email.record(c,{userId:u.id,purpose,to:u.email});await this.accounts.audit(c,actor,u.id,purpose+'_email_issued',{deliveryId});return {deliveryId,to:u.email,purpose,token,origin:this.origin};
 }
