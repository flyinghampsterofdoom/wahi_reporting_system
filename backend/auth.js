...
const CAPABILITIES = Object.freeze({
  ADMIN:[...inventoryManagement,'internal_cost.read','internal_cost.write','users.manage','domain.write','history.backdate','labor_rates.manage'],
...
function recipeView(r,revision) {return {id:r.id,name:r.name,outputIngredientId:r.outputIngredientId,
  revision:revision?{id:revision.id,outputQuantity:revision.outputQuantity,outputUnit:revision.outputUnit,
    lines:revision.lines.map(l=>({id:l.id,ingredientId:l.ingredientId,quantity:l.quantity,unit:l.unit})),steps:revision.steps.map(s=>({id:s.id,position:s.position,instruction:s.instruction}))}:null};}
function costView(result) {return {...result,completeCost:result.completeCost?.decimal()??null,knownSubtotal:result.knownSubtotal.decimal(),materialSubtotal:result.materialSubtotal.decimal(),laborSubtotal:result.laborSubtotal.decimal()};}
