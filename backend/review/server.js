...
function createReviewServer({service,users,integrations,runtime={hosted:false,environment:'local-review'},sessionStore,accounts,labor}){
  const sessions=new Map(),attempts=new Map();
  const assetNames=['app.js','style.css','management-ui.js','table-model.js','currency.js','shell-model.js','shell.js','users-ui.js','labor-ui.js'];
  ...
  const server=http.createServer(async(req,res)=>{
    ...
    try{
      ...
      if(p.startsWith('/api/labor')){
        ...
        if(p==='/api/labor/overview'&&req.method==='GET'){send(200,await labor.overview(actor,u.searchParams.get('date')));return;}
        if(p==='/api/labor/policy'&&req.method==='GET'){send(200,await labor.policy(actor));return;}
        if(p==='/api/labor/refresh'&&req.method==='POST'){const d=z.object({date:z.string().optional()}).strict().parse(await body(req));send(200,await labor.refresh(actor,d.date));return;}
...
if(require.main===module){(async()=>{
  const {openReview}=require('./runtime');const {service,pool,users,integrations,accounts,sessionStore,labor}=await openReview();
  const server=createReviewServer({service,users,integrations,accounts,sessionStore,labor});server.listen(4317,'127.0.0.1',()=>console.log('Wahi owner review: http://127.0.0.1:4317 (isolated local review)'));
