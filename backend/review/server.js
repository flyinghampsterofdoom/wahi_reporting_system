...
function createReviewServer({service,users,integrations,runtime={hosted:false,environment:'local-review'},sessionStore,accounts,labor,email,accountEmail,reports,cogs}){
  ...
  const assetNames=['app.js','style.css','management-ui.js','table-model.js','currency.js','quantity.js','shell-model.js','shell.js','users-ui.js','labor-ui.js','email-ui.js','leadership-summary.js','deep-links.js','reports-ui.js','cogs-ui.js','cogs-model.js','mapping-ui.js','composite-ui.js'];
  ...
  const mappingUiVersion=assets.then(entries=>crypto.createHash('sha256').update(entries.filter(a=>['mapping-ui.js','composite-ui.js','cogs-ui.js','quantity.js','app.js'].includes(a.name)).map(a=>a.url).join('|')).digest('hex').slice(0,16));
  ...
  const server=http.createServer(async(req,res)=>{
    ...
    try{
      ...
      if(p.startsWith('/api/reporting/cogs')){
        ...
        if(req.method==='POST'&&['mapping','mapping-bulk','mapping-settings','composite','composite-preview'].includes(p.split('/').at(-1))&&req.headers['x-wahi-mapping-ui']&&req.headers['x-wahi-mapping-ui']!==await mappingUiVersion)throw error('mapping_client_outdated');
