...
async function view(service,actor,kind,id){
 ...
 if(kind==='item'){
  ...
  if(can(actor,'internal_cost.read')){const w=require('./recipe-workflow');w.decorateCost(i.cost,s,at,can(actor,'internal_cost.write'));i.resolution=w.eligibility(s,id,at);i.dependentRecipes=w.dependents(s,id,at);}
 ...
 if(kind==='recipe'){const r=m.recipes.find(r=>r.id===id);if(!r)throw Object.assign(new Error('Recipe missing'),{code:'not_found'});if(can(actor,'internal_cost.read'))require('./recipe-workflow').decorateCost(r.cost,s,at,can(actor,'internal_cost.write'));return {recipe:r,categories:m.categories,references:s.ingredients.map(x=>({id:x.id,name:x.name,active:x.active}))};}
 ...
 return {categories:[],items:m.items.filter(i=>!i.finished).map(({tags,createdAt,updatedAt,...i})=>({...i,...(i.cost?{cost:compactCost(i.cost)}:{})}))};
}
async function references(service,actor,{inventoryOnly=false}={}){
 ...
 'recipeCategories',(SELECT COALESCE(jsonb_agg(jsonb_build_object('id',id,'name',name,'active',active,'sortOrder',sort_order)),'[]') FROM wahi_v2.recipe_categories WHERE NOT $2::boolean),
