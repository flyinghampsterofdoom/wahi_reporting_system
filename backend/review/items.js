...
async function itemRecord(service,actor,id){
 ...
 const item={...ingredientView(i),kind:basis?.kind==='recipe'?'Prepared Item':basis?.kind==='source'?'Source-yield Item':basis?.kind==='labor_only'?'Labor Item':r?'Prepared Item':basis?'Purchased Item':'Unresolved Item',recipe:r?recipeView(r,revision):null,inventory:s.inventoryItems.find(x=>x.ingredientId===id)?{baseUnit:s.inventoryItems.find(x=>x.ingredientId===id).baseUnit}:null,locations:s.inventoryAssignments.filter(a=>a.ingredientId===id).map(a=>({id:a.id,locationId:a.locationId,name:s.inventoryLocations.find(l=>l.id===a.locationId)?.name,countUnit:a.countUnit,active:a.active,sortOrder:a.sortOrder,version:a.version}))};
 if(can(actor,'internal_cost.read')){
  ...
  item.cost=cost(quantity||'1',unit||'each');if(!r&&item.cost.blockers.some(b=>b.ingredientId===id&&['missing_cost_basis','missing_yield'].includes(b.reason)))item.kind='Unresolved Item';item.costQuantity=quantity;item.costUnit=unit;item.commonCosts=['fl_oz_us','oz_wt','each'].filter(u=>u!==unit).map(u=>({unit:u,result:cost('1',u)})).filter(x=>x.result.completeCost!==null);
 }
 return item;
}
