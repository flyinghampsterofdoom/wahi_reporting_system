...
async function management(service,actor,{includeItems=true,includeMenuCosts=true,recipeId=null}={}){
 ...
 const items=(includeItems?s.ingredients:[]).map(i=>{
  ...
  const sourceYield=basis?.kind==='source'?at(s.yields.filter(y=>y.ingredientId===i.id)):null,unresolved=!basis||(basis.kind==='source'&&(!sourceYield?.sourceQuantity||!sourceYield?.sourceUnit||!sourceYield?.outputQuantity||!sourceYield?.outputUnit));
  const row={...ingredientView(i),itemKind:recipe?'Prepared':unresolved?'Unresolved':basis?.kind==='source'?'Source yield':basis?.kind==='labor_only'?'Labor':basis?'Purchased':'Unresolved',recipeId:recipe?.id||null,finished,locationCount:s.inventoryAssignments.filter(a=>a.ingredientId===i.id&&a.active&&s.inventoryLocations.some(l=>l.id===a.locationId&&l.active)).length,countable};
