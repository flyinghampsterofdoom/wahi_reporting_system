...
async function management(service,actor){
 ...
 const recipes=s.recipes.map(r=>{
  ...
  for(const m of s.menuItems){const mapping=at(s.menuMappings.filter(x=>x.menuItemId===m.id));if(mapping?.recipeId!==r.id)continue;const price=at(s.sellingPrices.filter(x=>x.menuItemId===m.id)),menu={id:m.id,name:m.name,active:m.active,quantity:mapping.quantity,unit:mapping.unit,sellingPrice:price?{amount:price.amount,currency:price.currency}:null};
   if(internal){const c=cost(r.outputIngredientId,mapping.quantity,mapping.unit);menu.cost=costView(c);menu.costPercentage=price?.currency==='USD'&&Exact.of(price.amount).positive()&&c.completeCost!==null?c.completeCost.div(price.amount).mul('100').decimal(4):null;}row.menus.push(menu);
  }
 ...
 const items=s.ingredients.map(i=>{
  const recipe=recipes.find(r=>r.outputIngredientId===i.id),basis=at(s.bases.filter(x=>x.ingredientId===i.id)),countable=s.inventoryItems.some(x=>x.ingredientId===i.id),usedAsInput=s.recipes.some(r=>at(s.recipeRevisions.filter(v=>v.recipeId===r.id))?.lines.some(l=>l.ingredientId===i.id))||s.yields.some(y=>at(s.yields.filter(x=>x.ingredientId===y.ingredientId))?.sourceIngredientId===i.id);
  // Finished, menu-linked outputs stay in Recipes unless also inventoried/used as inputs.
  const finished=!!recipe?.menus.some(m=>m.active)&&!countable&&!usedAsInput;
  ...
  if(internal){const p=s.purchaseOptions.find(p=>p.id===basis?.purchaseOptionId),price=p?at(s.prices.filter(x=>x.purchaseOptionId===p.id)):null;row.purchase=p?{label:p.label,quantity:p.contentQuantity,unit:p.contentUnit,price:price?{amount:price.amount,currency:price.currency}:null}:null;row.costQuantity=recipe?.revision?.outputQuantity||p?.contentQuantity||'1';row.costUnit=recipe?.revision?.outputUnit||p?.contentUnit||'each';row.cost=recipe?.cost||costView(cost(i.id,row.costQuantity,row.costUnit));}return row;
 });
